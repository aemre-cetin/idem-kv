...
To ensure stability across multi-step generation cycles without token oscillation, we formalize the target destination map through the algebraic framework of idempotent permutations~\cite{cetin2013idempotent}:
\begin{definition}[Idempotent Projection Map]
A mapping function $f: \{0, \dots, N-1\} \to \{0, \dots, N-1\}$ is an idempotent projection over the index space if and only if:
\begin{equation}
    f(f(x)) = f(x), \quad \forall x \in \{0, \dots, N-1\}
    \label{eq:idempotence}
\end{equation}
\end{definition}
Under Eq.~(\ref{eq:idempotence}), all elements mapped to their final designated positions become mathematical \textit{fixed points}:
\begin{equation}
    \mathcal{F} = \{x \mid f(x) = x\}
\end{equation}
As illustrated in Figure~\ref{fig:state}, elements within $\mathcal{F}$ represent stabilized attractor basins that do not undergo relocation in subsequent compaction epochs, ensuring single-pass convergence and preventing thrashing.
...
\end{equation}
with length $L_k \ge 1$ and $\mathcal{C}_j \cap \mathcal{C}_k = \emptyset$ for $j \ne k$. Trivial cycles of length $L_k = 1$ mirror precisely the fixed points $\mathcal{F}$ and require no data movement.
...
\begin{itemize}
    \item \textbf{Hardware Platform:} NVIDIA Ada Lovelace GPU Architecture (RTX 500 Generation, \texttt{sm\_89}).
...
\begin{table*}[t]
\centering
\caption{Empirical Hardware Compaction Performance Comparison on NVIDIA Ada Lovelace Architecture (\texttt{sm\_89})}
\label{tab:results}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lllll@{}}
\toprule
...
\subsection{Latency and Ablation Analysis}
Under realistic LLM KV-cache compaction (where evicted tokens are paired with retained tail tokens into 2-cycles and fixed points), our optimized V2 kernel achieves an execution latency of \textbf{7.79~ms}, outperforming the native PyTorch out-of-place gather baseline (\textbf{8.08~ms}) while completely eliminating memory allocation spikes. In the worst-case scenario with arbitrary long-cycle permutations, V2 reduces execution time from 60.03~ms to 20.82~ms, demonstrating a 2.88$\times$ speedup achieved through transposition fast paths and vectorized memory coalescing.
